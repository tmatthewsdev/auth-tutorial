...
class Auth_Login_Siteauth extends \Auth_Login_Driver
{
	...
	public static function _init()
	{
		...
		if (\Config::get('siteauth.remember_me.enabled', false))
		{
			static::$remember_me = \Session::forge(array(
				'driver' => 'cookie',
				'cookie' => array(
					'cookie_name' => \Config::get('siteauth.remember_me.cookie_name', 'rmcookie'),
				),
				'encrypt_cookie' => true,
				'expire_on_close' => false,
				'expiration_time' => \Config::get('siteauth.remember_me.expiration', 86400 * 31),
			));
		}
	}
	...
	protected function perform_check()
	{
		...
		if ( ! empty($username) and ! empty($login_hash))
		{
			...
			if ($this->user and (\Config::get('siteauth.multiple_logins', false) or $this->user['login_hash'] === $login_hash))
			{
				return true;
			}
		}
	...
	{
		$username_or_email = trim($username_or_email) ?: trim(\Input::post(\Config::get('siteauth.username_post_key', 'username')));
		$password = trim($password) ?: trim(\Input::post(\Config::get('siteauth.password_post_key', 'password')));
		...
		$user = \DB::select_array(\Config::get('siteauth.table_columns', array('*')))
			->where_open()
